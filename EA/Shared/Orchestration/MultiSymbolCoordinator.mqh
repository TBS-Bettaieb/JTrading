...
public:
   ...
   //+------------------------------------------------------------------+
   //| 🔥 NOUVEAU: Définir le multiplicateur de risque pour un symbole spécifique |
   //| Utilisé pour reproduire l'ordre exact de l'ancienne version     |
   //+------------------------------------------------------------------+
   void SetSymbolRiskMultiplier(string symbol, double multiplier)
   {
      int index = FindSymbolIndex(symbol);
      if(index >= 0 && m_symbols[index].trader != NULL)
      {
         m_symbols[index].trader.SetRiskMultiplier(multiplier);
         Logger::Debug("✅ Multiplier x" + DoubleToString(multiplier, 2) + " applied to " + symbol);
      }
   }
   
   //+------------------------------------------------------------------+
   //| 🔥 NOUVEAU: Traiter le tick pour un symbole spécifique          |
   //| Utilisé pour reproduire l'ordre exact de l'ancienne version     |
   //+------------------------------------------------------------------+
   void ProcessSymbolTick(string symbol)
   {
      ...
      if(index >= 0 && m_symbols[index].trader != NULL)
      {
         m_symbols[index].trader.OnTick();
      }
   }
   
   //+------------------------------------------------------------------+
   //| 🔥 NOUVEAU: Appliquer le trailing stop pour un symbole spécifique |
   //| Utilisé pour reproduire l'ordre exact de l'ancienne version     |
   //+------------------------------------------------------------------+
   void TrailSymbolStop(string symbol)
   {
      int index = FindSymbolIndex(symbol);
      if(index >= 0 && m_symbols[index].trader != NULL)
      {
         m_symbols[index].trader.TrailStop();
      }
   }
   
   //+------------------------------------------------------------------+
   //| 🔥 NOUVEAU: Appliquer le trailing TP pour un symbole spécifique |
   //| Utilisé pour reproduire l'ordre exact de l'ancienne version     |
   //+------------------------------------------------------------------+
   void ApplySymbolTrailingTP(string symbol)
   {
      int index = FindSymbolIndex(symbol);
      if(index >= 0 && m_symbols[index].trader != NULL)
      {
         m_symbols[index].trader.ApplyTrailingTP();
      }
   }
   
   //+------------------------------------------------------------------+
   //| 🔥 NOUVEAU: Annuler les ordres pending pour un symbole spécifique |
   //| Utilisé pour reproduire l'ordre exact de l'ancienne version     |
   //+------------------------------------------------------------------+
   void CancelSymbolPendingOrders(string symbol)
   {
      int index = FindSymbolIndex(symbol);
      if(index >= 0 && m_symbols[index].orderManager != NULL)
      {
         int cancelled = m_symbols[index].orderManager.DeleteAllOrders();
         if(cancelled > 0)
         {
            Logger::Debug("✅ " + IntegerToString(cancelled) + " order(s) cancelled for " + symbol);
         }
      }
   }
   
   //+------------------------------------------------------------------+
   //| 🔥 NOUVEAU: Obtenir le multiplicateur de risque d'un symbole    |
   ...
   double GetSymbolRiskMultiplier(string symbol)
   {
      ...
      if(index >= 0 && m_symbols[index].trader != NULL)
      {
         return m_symbols[index].trader.GetRiskMultiplier();
      }
      return 1.0;
   }
   
   //+------------------------------------------------------------------+
   //| 🔥 NOUVEAU: Vérifier si le trailing a été exécuté pour un symbole |
   //| Utilisé pour validation                                          |
   //+------------------------------------------------------------------+
   bool WasTrailingExecuted(string symbol)
   {
      int index = FindSymbolIndex(symbol);
      if(index >= 0 && m_symbols[index].trader != NULL)
      {
         // Vérifier si le trader a des positions ouvertes
         return m_symbols[index].positionManager.GetTotalPositions() > 0;
      }
      return false;
   }
