...
//+------------------------------------------------------------------+
//| Détecte le croisement baissier du MACD sur un timeframe donné.   |
//| Retourne true si le MACD vient de faire un croisement baissier   |
//| (histogramme ou ligne de signal).                                |
//| @param symbol Symbole à analyser                                  |
//| @param timeframe Timeframe d'analyse                             |
//| @param fast_period Période rapide MACD (défaut: 12)             |
//| @param slow_period Période lente MACD (défaut: 26)               |
//| @param signal_period Période signal MACD (défaut: 9)             |
//| @return true si croisement baissier détecté                      |
//+------------------------------------------------------------------+
static bool CheckMACDBearishCrossover(string symbol, ENUM_TIMEFRAMES timeframe, 
                                      int fast_period = 12, int slow_period = 26, int signal_period = 9)
{
   // Paramètres MACD standards
   int fastEMA = fast_period;
   ...
   // Avant: MACD était au-dessus ou égal à Signal
   // Maintenant: MACD est en dessous de Signal
   bool crossoverOccurred = (macdPrevious >= signalPrevious) && 
                          (macdCurrent < signalCurrent);
...
//+------------------------------------------------------------------+
//| Retourne true si le MACD est actuellement baissier, simple.     |
//| Vérifie l'état actuel du MACD sans analyser les croisements.    |
//| @param symbol Symbole à analyser                                  |
//| @param timeframe Timeframe d'analyse                             |
//| @param fast_period Période rapide MACD (défaut: 12)             |
//| @param slow_period Période lente MACD (défaut: 26)               |
//| @param signal_period Période signal MACD (défaut: 9)             |
//| @return true si MACD baissier                                     |
//+------------------------------------------------------------------+
static bool CheckMACDBearish(string symbol, ENUM_TIMEFRAMES timeframe,
                             int fast_period = 12, int slow_period = 26, int signal_period = 9)
{
   // Créer les arrays pour stocker les valeurs MACD
   double macdMain[];
   ...
   // Condition baissière: MACD Main Line en dessous de Signal Line
   bool isBearish = macdMainCurrent < macdSignalCurrent;
