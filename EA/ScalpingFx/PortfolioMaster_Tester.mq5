...
{
   Print("═══════════════════════════════════════════════════");
   Print("🎯 PORTFOLIO MASTER EA - CONFIG LOADER MODE");
   Print("═══════════════════════════════════════════════════");
   ...
   Print("💰 Initial Capital: $", DoubleToString(g_stats.initialBalance, 2));
   ...
   if(totalSymbols <= 0)
   {
      Print("❌ ERROR: No valid symbols found in list: ", InpSymbolsList);
      return(INIT_FAILED);
   }
   
   Print("📊 Portfolio Size: ", totalSymbols, " symbols");
   Print("───────────────────────────────────────────────────");
   ...
   for(int i = 0; i < totalSymbols; i++)
   {
      ...
      Print("🔍 Loading configuration for: ", symbol);
      
      // Validate symbol availability
      if(!ValidateSymbolAvailable(symbol))
      {
         Print("⚠️ WARNING: Symbol ", symbol, " not available - skipping");
         bots[i] = NULL;
         continue;
      }
      ...
      {
         Print("⚠️ WARNING: No configuration found for ", symbol, " - skipping");
         bots[i] = NULL;
         continue;
      }
      ...
      if(InpGlobalRiskPercent > 0)
      {
         ...
         Print("   ⚙️ Risk override: ", DoubleToString(originalRisk, 2), "% → ", 
      ...
      if(bots[i] == NULL)
      {
         Print("❌ ERROR: Failed to create bot for ", symbol);
         continue;
      }
      
      if(!bots[i].Initialize())
      {
         Print("❌ ERROR: Bot initialization failed for ", symbol);
         delete bots[i];
         bots[i] = NULL;
         continue;
      }
      ...
      Print("      Risk: ", DoubleToString(config.riskPercent, 2), "%");
      Print("      TP/SL: ", config.tpPoints, "/", config.slPoints, " pts");
      Print("      Mode: ", (config.strategyMode == STRATEGY_BREAKOUT ? "BREAKOUT" : "REVERSION"));
      Print("───────────────────────────────────────────────────");
   }
   ...
   if(activeBots == 0)
   {
      Print("❌ ERROR: No bots successfully initialized");
      CleanupBots();
      return(INIT_FAILED);
   }
   
   Print("═══════════════════════════════════════════════════");
   Print("✅ Portfolio Master initialized successfully");
   Print("📊 Active Bots: ", activeBots, "/", totalSymbols);
   Print("🚀 Ready to test portfolio trading");
   Print("═══════════════════════════════════════════════════");
...
{
   Print("═══════════════════════════════════════════════════");
   Print("🛑 PORTFOLIO MASTER EA - STOPPING");
   Print("═══════════════════════════════════════════════════");
   ...
   Print("───────────────────────────────────────────────────");
   Print("⏱️ Test Duration: ", testDuration, " days");
   ...
   Print("📉 Max Drawdown: $", DoubleToString(maxDrawdownAbs, 2),
         " (", DoubleToString(maxDrawdownPct, 2), "%)");
   Print("📊 Peak Balance: $", DoubleToString(g_stats.peakBalance, 2));
   Print("📉 Lowest Balance: $", DoubleToString(g_stats.lowestBalance, 2));
   Print("═══════════════════════════════════════════════════");
   ...
   Print("✅ Portfolio Master EA stopped successfully");
}
...
void UpdateStatistics()
{
   ...
   if(currentBalance > g_stats.peakBalance)
   {
      ...
      {
         Print("📈 New Peak Balance: $", DoubleToString(currentBalance, 2));
      }
   }
...
void SaveTestConfiguration(double totalProfit, double profitPercent, double maxDrawdownPct)
{
   ...
   {
      Print("❌ ERROR: Cannot create configuration file: ", filename);
      return;
   }
  ...
  // }
   ...
   Print("✅ Test configuration saved: ", filename);
}
